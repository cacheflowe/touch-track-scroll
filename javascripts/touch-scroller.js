 ...
 */
tts.TouchScroller = function( scrollOuterEl, scrollInnerEl, options ) {
    ...
    var onMove = function( touchEvent ) {
        ...
        if( _orientation == tts.TouchScroller.UNLOCKED ) {
            showScrollbars();
            tts.TouchScroller.activeScrollerInstance = _scrollerId;
        ...
        if( !_hasLockedDragAxis && _orientation != tts.TouchScroller.UNLOCKED ) {
            if( Math.abs( _touchTracker.touchmoved.x ) > _lockDirectionDragPixels ) decideDragAxis( tts.TouchScroller.HORIZONTAL );
            else if( Math.abs( _touchTracker.touchmoved.y ) > _lockDirectionDragPixels ) decideDragAxis( tts.TouchScroller.VERTICAL );
        } else {
            // scroll once we've decided a direction
            if( _orientation == tts.TouchScroller.UNLOCKED || ( _orientation == tts.TouchScroller.VERTICAL && _orientation == _dragLockAxis ) || ( _orientation == tts.TouchScroller.HORIZONTAL && _orientation == _dragLockAxis ) ) {
    ...
    var onEnd = function( touchEvent ) {
        ...
        setTimeout(function(){ 
            ...
            _dragLockAxis = null;
            _cancelClick = false;
            tts.TouchScroller.activeScrollerInstance = null; 
            tts.TouchScroller.innermostScrollerInstance = null;
        },1);

        if( tts.TouchScroller.activeScrollerInstance != _scrollerId ) {
            hideScrollbars();
            return;
        } 
        if( _timerActive == false ) return;
    ...
    var setOrientation = function( orientation ) {
        ...
        if( _orientation == tts.TouchScroller.VERTICAL ) {
            _scrollsX = false;
            _scrollsY = true;
            _axis = AXIS_Y;
            ...
            _curPosition.x = 0;
            if( _isPaged ) {
                if( prevOrientation == tts.TouchScroller.HORIZONTAL ) _pageIndex.y = _pageIndex.x;
                setPage( _pageIndex.y, true );
            }
        } else if( _orientation == tts.TouchScroller.HORIZONTAL ) {
            _scrollsX = true;
            _scrollsY = false;
            _axis = AXIS_X;
            ...
            _curPosition.y = 0;
            if( _isPaged ) {
                if( prevOrientation == tts.TouchScroller.VERTICAL ) _pageIndex.x = _pageIndex.y;
...
tts.TouchScroller.HORIZONTAL = 'horizontal';
tts.TouchScroller.VERTICAL = 'vertical';
tts.TouchScroller.UNLOCKED = null;
